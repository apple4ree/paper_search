...
/paper-search --with-figures               # 선택된 논문의 figure도 추출
...
`--with-figures` 플래그를 주면 선정된 논문의 PDF를 다운로드해 figure를 추출합니다.
두 가지 방법 중 자동 선택:
...
추출 결과는 `papers/<Venue>/<slug>/figures/fig-NN.png` + `fig-NN.txt` (caption).
