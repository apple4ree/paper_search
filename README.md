...
```
/paper-search                              # 기본값: 학회당 top 10, figure 자동 추출
/paper-search --top 15 --years 3           # 최근 3년, 학회당 15개
/paper-search --no-figures                 # figure 추출 건너뛰기 (더 빠름)
/paper-search "LLM agent planning"         # topic 문자열로 오버라이드
```

결과는 `./papers/` 에 저장됩니다. 각 학회 폴더 안에 논문 `.md` 파일과 동명의
asset 디렉토리 (`<slug>/figures/`) 가 한 쌍으로 만들어집니다.

### Figure 추출 방법 (default-on)

선정된 논문별로 PDF를 받아 figure를 추출합니다. 두 방법 중 자동 선택:

| 방법 | 품질 | 의존성 | 사용 시점 |
|---|---|---|---|
| **pymupdf** | 중 — caption 근접 매칭 | 순수 Python (기본) | `PDFFIGURES2_JAR` 미설정 |
| **pdffigures2** | 최상 — Semantic Scholar가 쓰는 gold standard | Java + 빌드한 jar | `PDFFIGURES2_JAR` 경로 export 시 |

pdffigures2를 쓰려면 (1회 setup):
```bash
git clone https://github.com/allenai/pdffigures2
cd pdffigures2 && sbt assembly
export PDFFIGURES2_JAR=$(pwd)/target/scala-2.12/pdffigures2-assembly-0.1.0.jar
```
...
papers/
└─ NeurIPS/
   ├─ 2024-smith-paper.md          # 논문 노트 (## Figures 섹션에서 참조)
   └─ 2024-smith-paper/            # 동명의 asset 디렉토리
      └─ figures/
         ├─ fig-01.png
         └─ fig-01.txt             # caption sidecar
```

Paywall/파싱 실패는 best-effort로 무시됩니다 — 해당 논문의 `## Figures` 섹션만
비워두고 파이프라인은 계속 진행.
...
│       │   ├── extract_figures_pymupdf.py # figure 추출 (approach A)
│       │   ├── extract_figures_pdffigures.py # figure 추출 (approach B)
│       │   └── get_figures.py             # orchestrator
